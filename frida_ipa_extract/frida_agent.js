...
var RTLD_GLOBAL = 8;
var dlopenFunc = null;
var opendirFunc = null;
var readdirFunc = null;
var closedirFunc = null;
var accessFunc = null;
var mkdirFunc = null;
var unlinkFunc = null;
var getenvFunc = null;
...
function getOpendir() {
  if (opendirFunc === null) {
    var ptr = findExport('opendir');
    if (ptr) {
      opendirFunc = new NativeFunction(ptr, 'pointer', ['pointer']);
    }
  }
  return opendirFunc;
}
...
function getReaddir() {
  if (readdirFunc === null) {
    var ptr = findExport('readdir');
    if (ptr) {
      readdirFunc = new NativeFunction(ptr, 'pointer', ['pointer']);
    }
  }
  return readdirFunc;
}
...
function getClosedir() {
  if (closedirFunc === null) {
    var ptr = findExport('closedir');
    if (ptr) {
      closedirFunc = new NativeFunction(ptr, 'int', ['pointer']);
    }
  }
  return closedirFunc;
}
...
function getAccess() {
  if (accessFunc === null) {
    var ptr = findExport('access');
    if (ptr) {
      accessFunc = new NativeFunction(ptr, 'int', ['pointer', 'int']);
    }
  }
  return accessFunc;
}
...
function getMkdir() {
  if (mkdirFunc === null) {
    var ptr = findExport('mkdir');
    if (ptr) {
      mkdirFunc = new NativeFunction(ptr, 'int', ['pointer', 'int']);
    }
  }
  return mkdirFunc;
}
...
function getUnlink() {
  if (unlinkFunc === null) {
    var ptr = findExport('unlink');
    if (ptr) {
      unlinkFunc = new NativeFunction(ptr, 'int', ['pointer']);
    }
  }
  return unlinkFunc;
}
...
function getGetenv() {
  if (getenvFunc === null) {
    var ptr = findExport('getenv');
    if (ptr) {
      getenvFunc = new NativeFunction(ptr, 'pointer', ['pointer']);
    }
  }
  return getenvFunc;
}
...
function tryLoadObjC() {
  ...
  if (typeof Module !== 'undefined') {
    ...
    if (dlopenFunc === null) {
      var ptr = findExport('dlopen');
      if (ptr) {
        dlopenFunc = new NativeFunction(ptr, 'pointer', ['pointer', 'int']);
      }
    }

    if (dlopenFunc) {
      try {
        dlopenFunc(allocUtf8String('/usr/lib/libobjc.A.dylib'), RTLD_NOW | RTLD_GLOBAL);
      } catch (e) {
        // Ignore and retry later.
      }
    }
